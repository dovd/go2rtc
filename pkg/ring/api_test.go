...
func TestParseAuthConfigQuotedWrappedToken(t *testing.T) {
	expected := &AuthConfig{RT: "raw-refresh-token", HID: "hardware-id"}
	b, err := json.Marshal(expected)
	if err != nil {
		t.Fatal(err)
...
func TestParseAuthConfigCLIOutputLine(t *testing.T) {
	expected := &AuthConfig{RT: "raw-refresh-token", HID: "hardware-id"}
	b, err := json.Marshal(expected)
	if err != nil {
		t.Fatal(err)
	...
	config, err := parseAuthConfig(`"refreshToken": "` + base64.StdEncoding.EncodeToString(b) + `"`)
	if err != nil {
		t.Fatal(err)
	}
